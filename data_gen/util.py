...
from third_party.xiuminglib import xiuminglib as xm    distributed_train_step_decor = distributed_train_step if debug else tf.function(distributed_train_step)




def manual_poses(poses):
    def p34_to_44(p):
        """p: Nx3x4."""
        return np.concatenate((p, np.tile(np.reshape(np.eye(4)[-1, :], (1, 1, 4)), (p.shape[0], 1, 1)),), 1)

    # for 0924-1 scene
    center = np.array([0.996229, 0.489369, -0.270198])
    x_axis = normalize(np.array([1.05954, -0.0367697, 0.732356]) - np.array([1.06365, 0.163903, 0.869414]))
    y_axis = normalize(np.array([1.02334, -0.788063, -0.557948]) - np.array([1.01817, -0.450538, -1.06878]))
    z_axis = normalize(np.cross(x_axis, y_axis))
    y_axis = normalize(np.cross(z_axis, x_axis))

    pos = center
    c2w = np.stack([x_axis, y_axis, z_axis, pos], 1)
    ...
    new_poses = np.concatenate([new_poses, np.broadcast_to(poses[:, :3, -1:].mean(0), new_poses[:, :3, -1:].shape)], -1)
    poses_reset = np.concatenate([poses_reset[:, :3, :4], poses[:, :3, -1:]], -1)
    return poses_reset, new_poses, sc
