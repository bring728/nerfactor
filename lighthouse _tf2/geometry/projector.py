...
from geometry import sampling as sampling
...
def mpi_resample_cube(mpi, tgt, intrinsics, depth_planes, side_length,
    ...
    coords_r = tf.reshape(tf.transpose(coords, [0, 4, 1, 2, 3]), [batch_size * cube_res, 4, cube_res, cube_res])
    ...
    intrinsics_tile_4 = tf.concat([intrinsics_tile, tf.zeros([batch_size * cube_res, 3, 1])], axis=2)
    ...
    coords_depth_inds = (tf.cast(num_depths, tf.float32) - 1) * ((1.0 / coords_depths) - (1.0 / depth_planes[0])) / (
                (1.0 / depth_planes[-1]) - (1.0 / depth_planes[0]))
    coords_proj = tf.concat([coords_proj, coords_depth_inds], axis=3)
    coords_proj = tf.transpose(tf.reshape(coords_proj, [batch_size, cube_res, cube_res, cube_res, 3]), [0, 2, 3, 1, 4])
...
def render_mpi_homogs(mpi_rgba, pose, newpose, min_disp, max_disp, num_depths, debug=False):
    ...
    H = plane_homogs(newpose, pose, 1. / dispvals, planes_from_t=False, y_flip=True, fix_yx=False)
    H = tf.reshape(H, [-1, 9])
    H = H[:, :8] / H[:, 8:]
    ...
    weights = mpiR_alpha * tf.cumprod(1. - mpiR_alpha + ALPHA_EPS, -2, exclusive=True, reverse=True)
...
def make_psv_homogs(img, pose, newpose, dispvals, num_depths, window=None):
    ...
    H = plane_homogs(newpose, pose, 1. / dispvals, planes_from_t=True, y_flip=True, fix_yx=False)  # [N, D, 3, 3]
