...
def trilerp_gather(vol, inds, bad_inds=None):
    ...
    invalid_x = tf.logical_or(tf.less(inds_x_0, 0.0), tf.greater(inds_x_1, tf.cast(tf.shape(vol)[2] - 1, tf.float32)))
    invalid_y = tf.logical_or(tf.less(inds_y_0, 0.0), tf.greater(inds_y_1, tf.cast(tf.shape(vol)[1] - 1, tf.float32)))
...
def bilerp_gather(img, inds):
    ...
    inds_b, _, _, = tf.meshgrid(tf.range(tf.shape(img)[0]), tf.range(tf.shape(img)[1]), tf.range(tf.shape(img)[2]),
                                indexing='ij')
    ...
    # store invalid indices to implement correct out-of-bounds conditions
    invalid_x = tf.logical_or(tf.less(inds_x_0, 0.0), tf.greater(inds_x_1, tf.cast(tf.shape(img)[2] - 1, tf.float32)))
    invalid_y = tf.logical_or(tf.less(inds_y_0, 0.0), tf.greater(inds_y_1, tf.cast(tf.shape(img)[1] - 1, tf.float32)))
    ...
    # boundary conditions for invalid indices
    invalid_inds = tf.tile(invalid_inds[:, :, :, tf.newaxis], [1, 1, 1, tf.shape(img)[3]])
