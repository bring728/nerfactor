...
class MLV(object):
    ...
    def infer_mpi(self, src_images, ref_image, ref_pose, src_poses, intrinsics, psv_planes):
        ...
        net_input = self.format_network_input(ref_image, src_images, ref_pose, src_poses, psv_planes, intrinsics)
    ...
    def img2mpi(self, img, depth, planedepths):
        ...
        depth_inds = (tf.to_float(num_depths) - 1) * ((1.0 / depth) - (1.0 / planedepths[0])) / (
                (1.0 / planedepths[-1]) - (1.0 / planedepths[0]))
    ...
    def predict_lighting_vol(self, mpi, planes, intrinsics, cube_res, scale_factors, depth_clip=20.0):
        ...
        for i in range(len(scale_factors)):
            i_center_depth = (cube_side_lengths[i] / (cube_res - 1)) * (cube_rel_shapes[i] // 2)
            cube_centers.append(tf.concat([tf.zeros([batchsize, 2]), i_center_depth * tf.ones([batchsize, 1])],
                                          axis=1))
    ...
    def render_envmap(self, cubes, cube_centers, cube_side_lengths, cube_rel_shapes, cube_nest_inds, ref_pose, env_pose,
    ...
    def format_network_input(self, ref_image, psv_src_images, ref_pose, psv_src_poses, planes, intrinsics):
