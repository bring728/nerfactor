...
def mpi_net(inputs):
    ...
    def conv(net, width, ksize=ksize, strides=1, d=1):
        ...
        return tf.keras.layers.Conv3D(width, ksize, strides=strides, padding='SAME', dilation_rate=(d, d, d),
                                      activation=None, kernel_initializer=initializer)(act(norm()(net)))
    ...
    def down_block(net, width, ksize=ksize, do_down=False, do_double=True):
        ...
        out = out + conv(conv(out, width, ksize), width, ksize) if do_double else out
        return out, out
    ...
    net = tf.keras.layers.Conv3D(width_list[0], 3, padding='SAME', activation=None, kernel_initializer=initializer)(net)
    for i in range(len(width_list)):
        net, skip = down_block(net, width_list[i], do_down=(i > 0), do_double=(i > 0))
        skips.append(skip)
    net = skips.pop()
    ...
    for i in range(len(width_list)):
        skip = skips.pop()
        net = up_block(net, skip, width_list[i], do_double=(i < len(width_list) - 1))

    # final 3d conv
    chout = 5  # bg RGB + alpha + weights
    net = tf.keras.layers.Conv3D(chout, 3, padding='SAME', activation=None, kernel_initializer=initializer)(
        act(norm()(net)))
...
def cube_net_multires(inputs, cube_rel_shapes, cube_nest_inds):
    ...
    def unet(net, width_list_down, width_list_up, chout):
        ...
        skips = []
        net = tf.keras.layers.Conv3D(width_list_down[0], 3, padding='SAME', activation=None,
                                     kernel_initializer=initializer)(net)

        for i in range(len(width_list_down)):
            net, skip = down_block(net, width_list_down[i], do_down=(i > 0), do_double=(i > 0))
            skips.append(skip)
        net = skips.pop()

        for i in range(len(width_list_up)):
            skip = skips.pop()
            net = up_block(net, skip, width_list_up[i], do_double=(i < len(width_list_up) - 1))
    ...
    for i in range(len(inputs)):
        ...
        i_outvol = tf.nn.sigmoid(i_net_out[Ellipsis, :-1]) * i_outvol_weights + inputs[i] * (1.0 - i_outvol_weights)
        ...
        if i < len(inputs) - 1:
            ...
            i_outvol_next = tf_repeat(i_outvol_next, [1,
