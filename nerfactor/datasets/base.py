...
class Dataset():
    def __init__(self, config, mode, debug=False, shuffle_buffer_size=64, prefetch_buffer_size=None,
                 n_map_parallel_calls=None):
        assert mode in ('train', 'vali', 'test'), (
                                                      "Accepted dataset modes: 'train', 'vali', 'test', but input is %s") % mode
        self.config = config
        self.mode = mode
        self.debug = debug
        if debug:
            logger.warn("Dataset in debug mode; behavior may be different")
        self.shuffle_buffer_size = self.config.getint('DEFAULT', 'shuffle_buffer_size', fallback=shuffle_buffer_size)
    ...
    def build_pipeline(self, filter_predicate=None, seed=None, no_batch=False, no_shuffle=False):
        ...
        dataset = dataset.map(self._process_example_precache, num_parallel_calls=self.n_map_parallel_calls)
        if cache:
            dataset = dataset.cache()
        # Useful if part of your processing involves randomness
        dataset = dataset.map(self._process_example_postcache, num_parallel_calls=self.n_map_parallel_calls)
        # Shuffle
