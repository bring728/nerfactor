...
class Dataset(BaseDataset):
    def __init__(
        ...
        assert len(test_paths) == 1, (
            "There should be a single set of test coordinates, shared by "            "all identities")
        self.brdf_names = [basename(x)[len('train_'):-len('.npz')] for x in train_paths]
    ...
    def _process_example_precache(self, path):
        ...
        id_, i, envmap_h, ims, spp, rusink, refl = tf.py_function(
            self._load_data, [path], (tf.string, tf.int32, tf.int32, tf.int32, tf.int32, tf.float32, tf.float32))
    ...
    def _process_example_postcache(self, id_, i, envmap_h, ims, spp, rusink, refl):
    ...
    def _sample_entries(self, tbls):
        ...
        for tbl in tbls:
            if n_rows is None:
                ...
                tf.debugging.assert_equal(tf.shape(tbl)[0], n_rows, message=(
                    "Tables must have the same number of rows to sample " "from"))
