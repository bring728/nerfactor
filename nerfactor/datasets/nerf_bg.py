...
class Dataset(BaseDataset):
    ...
    def _get_batch_size(self):
        if self.mode == 'train':
            ...
            map_data = ret[-2]  # OK as long as shape is (H, W[, ?])
    ...
    def _process_example_postcache(self, id_, rayo, rayd, rgb, radii, viewdir):
        """Records image dimensions and samples rays.
        """
        hw = tf.shape(rgb)[:2]
        rayo, rayd, rgb, radii, viewdir = self._sample_rays(rayo, rayd, rgb, radii, viewdir)
        ...
        return id_, hw, rayo, rayd, rgb, radii, viewdir

    def _sample_rays(self, rayo, rayd, rgb, radii, viewdir):
        ...
        if self.mode in ('vali', 'test') or self.always_all_rays:
            ...
            return rayo, rayd, rgb, radii, viewdir
        ...
        select_ind = tf.random.uniform((self.bs,), minval=0, maxval=tf.shape(coords)[0], dtype=tf.int32)
        ...
        radii = tf.gather_nd(radii, select_ind)
        viewdir = tf.gather_nd(viewdir, select_ind)
        return rayo, rayd, rgb, radii, viewdir

    def _process_example_precache(self, path):
        """Loads data from paths.
        """
        id_, rayo, rayd, rgb, radii, viewdir = tf.py_function(self._load_data, [path],
                                                     (tf.string, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32))
        return id_, rayo, rayd, rgb, radii, viewdir
    ...
    def _load_data(self, metadata_path):  # pylint: disable=arguments-differ
        ...
        rayo, rayd, radii, viewdir = self._gen_rays(cam_to_world, cam_angle_x, imh, imw)
        rayo, rayd, radii, viewdir = rayo.astype(np.float32), rayd.astype(np.float32), radii.astype(np.float32), viewdir.astype(np.float32)
        ...
        rgba = xm.io.img.load(img_path, self.config.getboolean('DEFAULT', 'auto_rotate'))
    ...
    def _gen_rays(self, to_world, angle_x, imh, imw):
        ...
        rayo = np.tile(cam_loc[None, None, :], (imh * self.sps, imw * self.sps, 1))
        # Ray directions
        ...
        # axis = self.config.get('DEFAULT', 'axis')
        # if axis == 'my':
        ...
        if ndc:
            ...
            rayo = rayo + t[..., None] * rayd
            # Projection
            o1 = -1. / (imw / (2. * fl)) * rayo[..., 0] / rayo[..., 2]
            o2 = -1. / (imh / (2. * fl)) * rayo[..., 1] / rayo[..., 2]
            o3 = 1. + 2. * near / rayo[..., 2]
            d1 = -1. / (imw / (2. * fl)) * (rayd[..., 0] / rayd[..., 2] - rayo[..., 0] / rayo[..., 2])
            d2 = -1. / (imh / (2. * fl)) * (rayd[..., 1] / rayd[..., 2] - rayo[..., 1] / rayo[..., 2])
            d3 = -2. * near / rayo[..., 2]
