...
class Dataset(BaseDataset):
    ...
    def _process_example_precache(self, path):
        ...
        """
        id_, rayo, rayd, radii, rgb = tf.py_function(self._load_data, [path], (tf.string, tf.float32, tf.float32, tf.float32, tf.float32))
        return id_, rayo, rayd, radii, rgb

    def _process_example_postcache(self, id_, rayo, rayd, radii, rgb):
        """Records image dimensions and samples rays.
        """
        hw = tf.shape(rgb)[:2]
        rayo, rayd,radii, rgb = self._sample_rays(rayo, rayd, radii, rgb)
        ...
        return id_, hw, rayo, rayd, radii, rgb

    def _sample_rays(self, rayo, rayd, radii,  rgb):
