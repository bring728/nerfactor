...
class Dataset(BaseDataset):
    ...
    def _glob(self):
        ...
        if self.debug:
            ...
            metadata_dir = join(root, '%s_001' % mode_str)
        else:
        ...
        for metadata_path in xm.os.sortglob(metadata_dir, 'metadata.json'):
            ...
            if self.mode == 'train':
                ...
                paths = {'xyz': xyz_path, 'alpha': alpha_path}
            if self.mode != 'test':
    ...
    def _process_example_precache(self, path):
        ...
        """
        id_, rayo, rayd, rgb, alpha, xyz, normal, lvis = tf.py_function(self._load_data, [path],
    ...
    def _process_example_postcache(self, id_, rayo, rayd, rgb, alpha, xyz, normal, lvis):
    ...
    def _sample_rays(self, rayo, rayd, rgb, alpha, xyz, normal, lvis, alpha_thres=0.9):
    ...
    def _load_data(self, metadata_path):
        ...
        if self.mode != 'train':
            if self.debug:
                ...
                lvis = 0.5 * np.ones(xyz.shape[:2] + (512,), dtype=np.float32)
                normal = 0.5 * np.ones_like(xyz, dtype=np.float32)
            else:
                normal = 0.5 * np.ones_like(xyz, dtype=np.float32)
                lvis = 0.5 * np.ones(xyz.shape[:2] + (512,), dtype=np.float32)
        else:
