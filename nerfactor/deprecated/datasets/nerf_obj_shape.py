...
from nerfactor.datasets.nerf_boundingbox import Dataset as BaseDataset
...
class Dataset(BaseDataset):
    ...
    def _glob(self):
        ...
        for metadata_path in xm.os.sortglob(metadata_dir, 'metadata.json'):
            ...
            xyz_local_path = join(nerf_root, id_, 'xyz_local.npy')
            xyz_world_path = join(nerf_root, id_, 'xyz_world.npy')
            alpha_path = join(nerf_root, id_, 'alpha.png')
            paths = {'xyz_local': xyz_local_path, 'xyz_world': xyz_world_path, 'normal': normal_path, 'lvis': lvis_path,
                     'alpha': alpha_path}
            if self.mode != 'test':
    ...
    def _process_example_precache(self, path):
        ...
        """
        id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis = tf.py_function(self._load_data, [path],
                                                                                             (tf.string, tf.int32,
                                                                                              ...
                                                                                              tf.float32, tf.float32,
                                                                                              tf.float32))
        return id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis

    # pylint: disable=arguments-differ
    def _process_example_postcache(self, id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis):
        """Records image dimensions and samples rays.
        """
        rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis = self._sample_rays(rayo, rayd, rgb, alpha,
                                                                                       xyz_local, xyz_world,
                                                                                       normal, lvis)

        # NOTE: some memory waste below to make distributed strategy happy
        id_ = tf.tile(tf.expand_dims(id_, axis=0), (tf.shape(alpha)[0],))
        hw = tf.tile(tf.expand_dims(hw, axis=0), (tf.shape(alpha)[0], 1))
        return id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis

    def _sample_rays(self, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis, alpha_thres=0.9):
        ...
        if self.mode in ('vali', 'test') or self.always_all_rays:
            ...
            xyz_local = tf.reshape(xyz_local, (-1, 3))
            xyz_world = tf.reshape(xyz_world, (-1, 3))
            normal = tf.reshape(normal, (-1, 3))
            lvis = tf.reshape(lvis, (-1, 512))
            return rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis
        # self.bs = 1024
        ...
        if alpha_thres is None:
            ...
            alpha_thres_ind = tf.where(alpha > alpha_thres)[:, 0]
            idxs = tf.range(tf.shape(alpha_thres_ind)[0])
            ridxs = tf.random.shuffle(idxs)[:self.bs]
            select_ind = tf.gather(alpha_thres_ind, ridxs)
        ...
        alpha = tf.reshape(alpha, (-1, 1))
        xyz_local = tf.gather(xyz_local, select_ind)
        xyz_world = tf.gather(xyz_world, select_ind)
    ...
    def _load_data(self, metadata_path):
        ...
        xyz_local = ioutil.load_np(paths['xyz_local'])
        xyz_world = ioutil.load_np(paths['xyz_world'])
        ...
        # Resize
        if imh != xyz_local.shape[0]:
            xyz_local = xm.img.resize(xyz_local, new_h=imh)
            xyz_world = xm.img.resize(xyz_world, new_h=imh)
        ...
        assert np.isclose(np.linalg.norm(normal, axis=2), 1).all(), "Found normals with a norm far away from 1"
        ...
        alpha = alpha[mask]
        xyz_local = np.reshape(xyz_local, (-1, 3))
        xyz_local = xyz_local[mask]
        xyz_world = np.reshape(xyz_world, (-1, 3))
        xyz_world = xyz_world[mask]
        normal = np.reshape(normal, (-1, 3))
        normal = normal[mask]
        lvis = np.reshape(lvis, (-1, 512))
        lvis = lvis[mask]
        # Make sure there's no XYZ coinciding with camera (caused by occupancy accumulating to 0)
        assert not np.isclose(xyz_world, rayo).all(
            axis=1).any(), "Found XYZs coinciding with the camera" + self.mode + id_
        return id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis
