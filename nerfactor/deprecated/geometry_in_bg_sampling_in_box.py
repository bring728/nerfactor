...
def main(_):
    ...
    config.set('DEFAULT', 'ray_box_threshold', str(ray_box_threshold))
    ...
    dataset = Dataset(config, mode, always_all_rays=True, spp=spp, obj_3d_bb=obj_3d_bb)
    ...
    for batch in tqdm(datapipe, desc=f"Views ({mode})", total=n_views):
        ...
        process_view(config, model, batch, w2l_mat, w2l_tr, dataset.dict_mask[id])


def process_view(config, model, batch, w2l_mat, w2l_tr, mask):
    ...
    expected = [join(out_dir, 'alpha.png'), join(out_dir, 'xyz.npy'), join(out_dir, 'xyz.png'), join(out_dir, 'depth.npy'), join(out_dir, 'depth.png')]
    ...
    occu_coarse, exp_depth_coarse, occu_fine, exp_depth_fine = compute_depth(model, rayo, rayd, tmin, tmax, config,
                                                                             w2l_mat, w2l_tr)
    occu = occu_fine
    exp_depth = exp_depth_fine
    ...
    ind = tf.where(mask)
    img_tmp = tf.zeros(hw)
    occu = tf.tensor_scatter_nd_update(img_tmp, ind, occu)
    ...
    surf = tf.linalg.matmul(surf, w2l_mat) + w2l_tr[None, :]

    img_tmp = tf.zeros(tf.concat([hw, tf.constant(3, shape=1)], axis=0))
    surf = tf.tensor_scatter_nd_update(img_tmp, ind, surf)
    ...
    img_tmp = tf.zeros(hw)
    exp_depth = tf.tensor_scatter_nd_update(img_tmp, ind, exp_depth)
...
def compute_depth(model, rayo, rayd, tmin, tmax, config, w2l_mat, w2l_tr):
    ...
    pts = tf.linalg.matmul(pts, w2l_mat) + w2l_tr[None, None, :]
...
def check_bounds(pts):
    ...
    # Parse bounds
    x_min, x_max, y_max, y_min, z_max, z_min = scene_bbox.split(',')
