...
from nerfactor.util import logging as logutil, io as ioutil, img as imgutil, tensor as tutil, math as mathutil
...
class Model(BaseModel):
    def __init__(self, config, debug=False):
        ...
        # ------ Shape
        self.normal_smooth_weight = self.config.getfloat('DEFAULT', 'normal_smooth_weight', fallback=0.)
        self.lvis_smooth_weight = self.config.getfloat('DEFAULT', 'lvis_smooth_weight', fallback=0.)
        # ------ Embedders
    ...
    def _init_embedder(self):
        ...
        if not pos_enc:
            embedder = {'xyz': tf.identity, 'ldir': tf.identity, 'vdir': tf.identity}
            return embedder
        # Position embedder
        kwargs = {
            'incl_input': True,
        ...
        #
        embedder = {'xyz': embedder_xyz, 'ldir': embedder_ldir, 'vdir': embedder_vdir}
        return embedder

    def _calc_ldir(self, pts):
        surf2l = tf.reshape(self.lxyz, (1, -1, 3)) - pts[:, None, :]
        surf2l = mathutil.safe_l2_normalize(surf2l, axis=2)
        tf.debugging.assert_greater(tf.linalg.norm(surf2l, axis=2), 0., message="Found zero-norm light directions")
        return surf2l  # NxLx3
    ...
    def _calc_vdir(cam_loc, pts):
        ...
        tf.debugging.assert_greater(tf.linalg.norm(surf2c, axis=1), 0., message="Found zero-norm view directions")
        return surf2c  # Nx3

    def call(self, batch, mode='train'):
        xyz_jitter_std = self.config.getfloat('DEFAULT', 'xyz_jitter_std')
        self._validate_mode(mode)
        id_, hw, rayo, rayd, rgb, alpha, xyz_local, xyz_world, normal, lvis = batch
        surf2l = self._calc_ldir(xyz_local)
        # Jitter XYZs
        if xyz_jitter_std > 0:
            xyz_noise = tf.random.normal(tf.shape(xyz_local), stddev=xyz_jitter_std)
        else:
            xyz_noise = None

        # ------ Normals
        normal_pred = self._pred_normal_at(xyz_local)
        if xyz_noise is not None and self.normal_smooth_weight > 0:
            normal_jitter = self._pred_normal_at(xyz_local + xyz_noise)
        else:
            normal_jitter = None
        ...
        # ------ Light visibility
        lvis_pred = self._pred_lvis_at(xyz_local, surf2l)
        if xyz_noise is not None and self.lvis_smooth_weight > 0:
            lvis_jitter = self._pred_lvis_at(xyz_local + xyz_noise, surf2l)
        else:
            lvis_jitter = None
        # ------ Loss
    ...
    def _pred_normal_at(self, pts, eps=1e-6):
        ...
        tf.debugging.assert_greater(tf.linalg.norm(normal, axis=1), 0., message="Found zero-norm normals")
        return normal  # Nx3
    ...
    def compute_loss(self, pred, gt, **kwargs):
        ...
        """
        normal_loss_weight = self.config.getfloat('DEFAULT', 'normal_loss_weight')
    ...
    def vis_batch(self, data_dict, outdir, mode='train', dump_raw_to=None):
        ...
        for k, v in data_dict.items():
            ...
            if k.endswith('normal'):
                ...
                img_tmp = np.zeros(hw + (1,))
                img_tmp[np.where(mask)] = v.numpy()
                v_ = img_tmp[..., 0]
            elif k.endswith('lvis'):
                img_tmp = np.zeros(hw + (v_.shape[1],))
                img_tmp[np.where(mask)] = v.numpy()
                v_ = img_tmp
            else:
        ...
        for k, v in data_dict.items():
            if k.endswith('normal'):
                ...
                img_dict[k] = xm.io.img.write_arr(v, join(outdir, k + '.png'), clip=True)
            else:
                raise NotImplementedError(k)
