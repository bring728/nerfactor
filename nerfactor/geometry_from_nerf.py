...
scene = '0924-1'
imh = None
# scene_bbox = '-0.3,0.3,-0.3,0.4,-0.7,0.5' # 0908
# scene_bbox = '-0.3,0.3,-0.35,0.35,-0.15,0.15' #0917
scene_bbox = '-0.2,0.2,-0.2,0.2,-0.0,0.6'  # 0924-1
# scene_bbox = '-0.5,0.5,-0.5,0.5,-0.1,0.2' #0924-2

data_root = root_dir + 'real_scene/' + scene
out_root = root_dir + 'output/surf/' + scene + '_nerf'
lr = '5e-4'
trained_nerf = root_dir + "output/train/" + scene + '/' + scene + "_nerf/lr5e-4_batch2048_far2.0"
occu_thres = '0.5'
...
flags.DEFINE_integer('lpix_chunk', 1, (
    ...
    for mode in ('train', 'vali', 'test',):
        ...
        for batch in tqdm(datapipe, desc=f"Views ({mode})", total=n_views):
            if parallel_view:
                ...
                if id % num_gpu == int(gpus):
                    process_view(config, model, batch, mode)
            else:
                process_view(config, model, batch, mode)


def process_view(config, model, batch, mode):
    sps = int(np.sqrt(FLAGS.spp))  # no need to check if square

    if len(batch) == 5:
        id_, hw, rayo, rayd, _ = batch
    else:
        id_, hw, rayo, rayd, _, _ = batch
    ...
    if mode == 'train':
        compute_geometry = compute_depth_and_normal
        expected = [
        ...
        compute_geometry = compute_depth
        expected = [
            join(out_dir, 'alpha.png'),
            join(out_dir, 'xyz.npy'), join(out_dir, 'xyz.png')]
    ...
    # occu, exp_depth, exp_normal = compute_depth_and_normal_my(model, rayo, rayd, config)
    occu, exp_depth, exp_normal = compute_geometry(model, rayo, rayd, config)
...
def compute_depth(model, rayo, rayd, config):
    ...
    weights = model.accumulate_sigma(sigma, z, rayd, is_activated=True, accu_chunk=131072)
...
def compute_light_visibility(model, surf, normal, config, lvis_near=.1):
    ...
    for i in range(0, n_lights, FLAGS.lpix_chunk):
        ...
        z = model.gen_z(  # NOTE: start from lvis_near instead of 0
            ...
            perturb=perturb)
        pts = surf_flat_frontlit[:, None, :] + surf2l_flat_frontlit[:, None, :] * z[:, :, None]
        pts_flat = tf.reshape(pts, (-1, 3))
        sigma_flat = eval_sigma_mlp(model, pts_flat, use_fine=False)
        sigma = tf.reshape(sigma_flat, pts.shape[:2])
        weights = model.accumulate_sigma(sigma, z, surf2l_flat_frontlit, is_activated=True, accu_chunk=131072)
...
def compute_depth_and_normal(model, rayo, rayd, config):
    ...
    for i in range(0, pts_flat.shape[0], FLAGS.mlp_chunk):
        ...
        with tf.GradientTape() as g:
            ...
            sigma_chunk = model.density_activation(fine_sigma_out(fine_enc(embedder(pts_chunk))) + model.density_bias)
...
def compute_depth_and_normal_my(model, rayo, rayd, config):
    n_samples_coarse = 64 + config.getint('DEFAULT', 'n_samples_coarse')
    n_samples_fine = 64 + config.getint('DEFAULT', 'n_samples_fine')
    lin_in_disp = config.getboolean('DEFAULT', 'lin_in_disp')
    perturb = False  # NOTE: do not randomize at test time
    ...
    # For real scenes: sigma out of bounds should be 0
    in_bounds = check_bounds(pts_flat)
    pts_in = tf.boolean_mask(pts_flat, in_bounds)
    embedder = model.embedder['xyz']
    fine_enc = model.net['fine_enc']
    fine_sigma_out = model.net.get('fine_a_out', model.net['fine_sigma_out'])
    ...
    full_shape = (tf.shape(pts_flat)[0], 1)
    in_ind = tf.where(in_bounds)
    sigma = tf.scatter_nd(in_ind, sigma_in, full_shape)
    assert normal_chunks, "No normal chunk to concat."
    normal_in = tf.concat(normal_chunks, axis=0)
    full_shape = (tf.shape(pts_flat)[0], 3)
    normal = tf.scatter_nd(in_ind, normal_in, full_shape)

    sigma = tf.reshape(sigma, pts.shape[:2])  # (n_rays, n_samples)
    normal = tf.reshape(normal, pts.shape)  # (n_rays, n_samples, 3)
...
def eval_sigma_mlp(model, pts, use_fine=False):
    ...
    for i in range(0, pts_in.shape[0], FLAGS.mlp_chunk):
        ...
        sigma_chunk = model.density_activation(sigma_out(enc(embedder(pts_chunk))) + model.density_bias)
        sigma_chunks.append(sigma_chunk)
    assert sigma_chunks, "No sigma chunk to concat."
    sigma_in = tf.concat(sigma_chunks, axis=0)
