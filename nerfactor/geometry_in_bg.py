...
def get_rot_mat(rad, axis):
    if axis is 'x':
        return np.array([[1, 0, 0], [0, np.cos(rad), -np.sin(rad)], [0, np.sin(rad), np.cos(rad)]], dtype=np.float32)
    elif axis is 'y':
        return np.array([[np.cos(rad), 0, np.sin(rad)], [0, 1, 0], [-np.sin(rad), 0, np.cos(rad)]], dtype=np.float32)
    elif axis is 'z':
        return np.array([[np.cos(rad), -np.sin(rad), 0], [np.sin(rad), np.cos(rad), 0], [0, 0, 1]], dtype=np.float32)


def get_path(init_obj_loc, o2b_S, rot_axis_mat, n_step, step_length_x, step_length_z, rotation=True, n_rot_step=40):
    init_obj_loc = np.squeeze(init_obj_loc)
    y = init_obj_loc[1]
    xs = np.linspace(init_obj_loc[0] - step_length_x, init_obj_loc[0] + step_length_x, n_step, endpoint=True)
    zs = np.linspace(init_obj_loc[2] - step_length_z, init_obj_loc[2] + step_length_z, n_step, endpoint=True)

    if rotation:
        rot_z = np.linspace(0, np.radians(360), n_rot_step, endpoint=True)
    else:
        rot_z = np.zeros(n_rot_step)
    ...
    for i_z, z in enumerate(zs):
        for i_x, x in enumerate(xs):
            obj_loc = np.array([x, y, z]).reshape([3, 1])
            o2b_R = o2b_S @ get_rot_mat(rot_z[(i_x + i_z * n_step) % n_rot_step], axis='y') @ rot_axis_mat
            ...
            b2o_mat_list.append(b2o_proj[:3, :3])
            b2o_tr_list.append(b2o_proj[:3, 3])
        xs = np.flip(xs)

    return obj_loc_list, b2o_mat_list, b2o_tr_list
...
def main(_):
    ...
    obj_loc = np.array([0, 0.2, 2.0]).reshape([3, 1])
    rot_x = np.radians(0)
    rot_y = np.radians(0)
    rot_z = np.radians(0)
    ...
    rot_axis_mat = get_rot_mat(rot_x_axis, axis='x')
    o2b_R = get_rot_mat(rot_z, axis='z') @ get_rot_mat(rot_x, axis='x') @ get_rot_mat(rot_y, axis='y') @ rot_axis_mat
    o2b_R = o2b_S @ o2b_R
    ...
    if view_fixed:
        n_step = 15
        rotation = False
        obj_loc_list, b2o_mat_list, b2o_tr_list = get_path(obj_loc, o2b_S, rot_axis_mat, n_step=n_step,
                                                           step_length_x=0.4,
                                                           step_length_z=0.5, rotation=rotation, n_rot_step=40)
        fixed_view = datapipe.take(1)
        for batch in tqdm(fixed_view, desc=f"Views ({mode})", total=1):
            id = batch[0][0].numpy().decode()
            print(id)
            for index, (mat, tr, loc) in tqdm(enumerate(zip(b2o_mat_list, b2o_tr_list, obj_loc_list)),
                                              total=n_step * n_step):
                if index % 2 == int(gpus):
                    mat = tf.convert_to_tensor(mat, dtype=tf.float32)
                    tr = tf.convert_to_tensor(tr, dtype=tf.float32)
                    process_view(config, model, batch, mat, tr, loc, f'loc_{index:03d}', id)
            break

    else:
        for batch in tqdm(datapipe, desc=f"Views ({mode})", total=n_views):
            id = batch[0][0].numpy().decode()
            if int(id.split('_')[1]) % 2 == int(gpus):
                process_view(config, model, batch, w2l_mat, w2l_tr, obj_loc)


def process_view(config, model, batch, w2l_mat, w2l_tr, obj_loc, index=None, id=None):
    ...
    if id is not None:
        view_dir = join(bg_datadir, id)
        shutil.copy(join(view_dir, 'metadata.json'), join(out_dir, 'metadata.json'))
        shutil.copy(join(view_dir, 'nn.png'), join(out_dir, 'nn.png'))
