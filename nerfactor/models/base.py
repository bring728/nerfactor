...
class Model(tf.keras.Model):
    ...
    def __init__(self, config, debug=False):
        ...
        # of your model into this dictionary, values of which will be registered as trainable
        self.trainable_registered = False  # NOTE: before training, call
        # register_trainable() to register trainable parameters (which lie in self.net)
    ...
    def register_trainable(self):
        ...
        for net_name, net in self.net.items():
            ...
            assert attr_name.isidentifier(), (
                "Prepending '{pref}' to your network name '{net}' doesn't make a valid identifier; change your network name").format(
            ...
            for layer_i, layer in enumerate(net.layers):
                if layer.trainable:
                    ...
                    assert not hasattr(self, attr_name_full), (
                        "Can't register `{}` because it is already an attribute").format(attr_name_full)
