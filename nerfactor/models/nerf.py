...
class Model(BaseModel):
    ...
    @staticmethod
    def gen_z_box(near, far, n_samples, lin_in_disp=False, perturb=False):
    ...
    def _render_rays(self, rayo, rayd, mode='train'):
        ...
        z = self.gen_z(self.near, self.far, n_samples_coarse, rayo.shape[0],
        ...
        pts = rayo[:, None, :] + rayd[:, None, :] * z[:, :, None]  # (n_rays, n_samples_coarse + n_samples_fine, 3)
        views = tf.broadcast_to(rayd[:, None, :], pts.shape)
    ...
    @staticmethod
    def accumulate_sigma(sigma, z, rayd, noise_std=0., inf=1e10, accu_chunk=65536):
        # Compute "distance" (in time) between each integration time along a ray
        dist = z[:, 1:] - z[:, :-1]
        # The "distance" from the last integration time is infinity
        dist = tf.concat((dist, tf.broadcast_to([inf], dist[:, :1].shape)), axis=-1)
        dist = dist * tf.linalg.norm(rayd[:, None, :], axis=-1)  # already normalized
        # Add noise to model's predictions for density. Can be used to
        # regularize network during training (prevents floater artifacts)
        noise = tf.random.normal(sigma.shape) * noise_std
        # Predict density of each sample along each ray. Higher values imply
        # higher likelihood of being absorbed at this point
        density = 1.0 - tf.exp(-tf.nn.relu(sigma + noise) * dist)  # NOTE
        ...
        for i in range(0, density.shape[0], accu_chunk):
            ...
            weights_chunk = density_chunk * mathutil.safe_cumprod(1. - density_chunk)  # (n_rays_chunk, n_samples)
    ...
    def _compile_into_video(self, batch_dirs, out_mp4, fps=12):
        ...
        for batch_dir in tqdm(batch_dirs, desc="Compiling visualized batches"):
            ...
            pred_depth_path = join(batch_dir, 'fine_depth.png')
            ...
            pred_depth = xm.io.img.load(pred_depth_path)
            ...
            if n_nn == 0:
                frame = pred
                depth_frame = pred_depth
            elif n_nn == 1:
                nn_path = nn_paths[0]
                nn = xm.io.img.load(nn_path)
                frame = imgutil.hconcat((pred, nn))
                depth_frame = imgutil.hconcat((pred_depth, nn))
            else:
                raise RuntimeError((
                                       "There must be either zero or one nearest neighbor for "
                                       "each test camera, but found %d") % n_nn)
            frames[id_] = frame
            depth_frames[id_] = depth_frame
        # Make video
        frames_sorted = [frames[k] for k in sorted(frames)]
        depth_frames_sorted = [depth_frames[k] for k in sorted(depth_frames)]
        xm.vis.video.make_video(frames_sorted, fps=fps, outpath=out_mp4)
        out_mp4 = out_mp4.split('.')
        out_mp4 = out_mp4[0] + '_depth.' + out_mp4[1]
        xm.vis.video.make_video(depth_frames_sorted, fps=fps, outpath=out_mp4)
