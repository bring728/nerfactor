...
logger = logutil.Logger(loggee="models/nerf_bg")
...
class Model(BaseModel):
    def __init__(self, config, debug=False):
        ...
        self.ndc = self.config.getboolean('DEFAULT', 'ndc')
        if self.ndc:
            self.near = 0.0
            self.far = 1.0
        else:
    ...
    def call(self, batch, mode='train'):
        ...
        id_, hw, rayo, rayd, rgb, radii, viewdir = batch  # all flattened
        pred_coarse, pred_fine = self._render_rays(hw[0], rayo, rayd, radii, viewdir, mode=mode)
    ...
    def obj_call(self, rayo, rayd, radii):
        _, pred_fine = self._render_rays(rayo, rayd, radii, mode='vali')
        return pred_fine['rgb'], pred_fine['occu'], pred_fine['depth'], pred_fine['disp']

    def mlv_call(self, pts_flat, views_flat, diag_covs, voxel_size):
        if self.ipe:
            diag_covs = tf.tile(tf.expand_dims(diag_covs, axis=0), tf.shape(pts_flat))
            rgbs = self._eval_nerf_at_ipe(pts_flat, diag_covs, views_flat, reshape=False)
        else:
            rgbs = self._eval_nerf_at(pts_flat, views_flat, use_fine=True, reshape=False)

        sigma = rgbs[:, 3:]
        alpha = 1.0 - tf.exp(-self.density_activation(sigma + self.density_bias) * voxel_size)  # (n_rays, n_samples)
        rgb = rgbs[:, :3]
    ...
    def nerfactor_call(self, rayo, rayd, radii, view_dir):
        if self.ipe:
            z = self.gen_z(self.near, self.far, self.n_samples_coarse, rayo.shape[0])
        ...
        _, _, _, _, weights = self._accumulate(rgbs, z, rayd)

        if self.ipe:
            z = self.gen_z_fine_ipe(z, weights, self.n_samples_fine)
            mean, covs = self.ipe_cast(z, rayo, rayd, radii)
            views = tf.broadcast_to(view_dir[:, None, :], mean.shape)
            rgbs = self._eval_nerf_at_ipe(mean, covs, views)
        else:
            z = self.gen_z_fine(z, weights, self.n_samples_fine)
            pts = rayo[:, None, :] + rayd[:, None, :] * z[:, :, None]  # (n_rays, n_samples, 3)
            views = tf.broadcast_to(view_dir[:, None, :], pts.shape)
    ...
    def _render_rays(self, hw, rayo, rayd, radii, view_dir, mode='train'):
        ...
        if self.ipe:
            z = self.gen_z(self.near, self.far, self.n_samples_coarse, rayo.shape[0], lin_in_disp=lin_in_disp,
                           perturb=False)
            mean, covs = self.ipe_cast(z, rayo, rayd, radii)
            views = tf.broadcast_to(view_dir[:, None, :], mean.shape)
            rgbs = self._eval_nerf_at_ipe(mean, covs, views)
        ...
        rgb, occu, depth, disp, weights = self._accumulate(rgbs, z, rayd, perturb=perturb)
    ...
    def accumulate_sigma(self, sigma, z, rayd, noise_std=0., inf=1e10, accu_chunk=65536):
    ...
    def _accumulate(self, rgbs, z, rayd, eps=1e-10, perturb=False):
    ...
    def accumulate_with_obj(self, bg_rgbs, obj_rgb, bg_z, obj_z, rayd, eps=1e-10, inf=1e10):
        num_rays = tf.shape(bg_z)[0]
        if self.ipe:
            num_samples = tf.shape(bg_z)[1]
        else:
            num_samples = tf.shape(bg_z)[1] + 1
        bg_rgb = bg_rgbs[:, :, :3]
        bg_rgb = tf.math.sigmoid(bg_rgb)
        if self.widened:
            bg_rgb = bg_rgb * (1 + 2 * self.rgb_padding) - self.rgb_padding

        bg_sigma = bg_rgbs[:, :, 3]

        obj_ind = tf.searchsorted(bg_z, obj_z, side='left')
        obj_ind = tf.clip_by_value(obj_ind, 0, num_samples-1)
        obj_ind = tf.one_hot(obj_ind, num_samples, on_value=True, off_value=False, dtype=tf.bool)[:, 0, :]
        bg_ind = tf.math.logical_not(obj_ind)
        bg_ind = tf.cast(tf.where(bg_ind), tf.int32)
        obj_ind = tf.cast(tf.where(obj_ind), tf.int32)

        bg_rgb = tf.reshape(bg_rgb, (-1, 3))
        rgb = tf.zeros((num_rays,num_samples,3))
        # tf.scatter_nd(obj_ind, obj_rgb, (num_rays, num_samples,3))
        rgb = tf.tensor_scatter_nd_update(rgb, bg_ind, bg_rgb)
        rgb = tf.tensor_scatter_nd_update(rgb, obj_ind, obj_rgb)
        ...
        dist = bg_z[:, 1:] - bg_z[:, :-1]
        # The "distance" from the last integration time is infinity
        if dist.shape != bg_sigma.shape:
            dist = tf.concat((dist, tf.broadcast_to([inf], dist[:, :1].shape)), axis=-1)
        # should be redundant because rayd is# already normalized
        dist = dist * tf.linalg.norm(rayd[:, None, :], axis=-1)
        # Predict density of each sample along each ray. Higher values imply
        # higher likelihood of being absorbed at this point
        bg_alpha = 1.0 - tf.exp(-self.density_activation(bg_sigma + self.density_bias) * dist)
        bg_alpha = tf.reshape(bg_alpha, (-1, 1))
        alpha = tf.ones((num_rays,num_samples,1))
        alpha = tf.tensor_scatter_nd_update(alpha, bg_ind, bg_alpha)
        alpha = tf.reshape(alpha, (num_rays,num_samples))

        # Chunk by chunk to avoid OOM
        weights_chunks = []
        for i in range(0, alpha.shape[0], accu_chunk):
            end_i = min(alpha.shape[0], i + accu_chunk)
            density_chunk = alpha[i:end_i, :]
            # cumprod() is used to express the idea of the ray not having
            # reflected up to this sample yet
            weights_chunk = density_chunk * mathutil.safe_cumprod(1. - density_chunk)
            weights_chunks.append(weights_chunk)
    ...
    def vis_batch(self, data_dict, outdir, mode='train', dump_raw_to=None, text_loc_ratio=0.05, text_size_ratio=0.05):
    ...
    def compile_batch_vis(self, batch_vis_dirs, outpref, mode='train', fps=12):
        ...
        if mode == 'vali':
            ...
            outpath = outpref + '_depth.mp4'
            self._compile_into_depth_video(batch_vis_dirs, outpath, fps=fps)
    ...
    def _compile_into_depth_video(self, batch_dirs, out_mp4, fps=12):
        data_root = self.config.get('DEFAULT', 'data_root')
        # Load frames
        frames = {}
        for batch_dir in tqdm(batch_dirs, desc="Compiling visualized batches"):
            json_path = join(batch_dir, 'metadata.json')
            pred_path = join(batch_dir, 'fine_depth.png')
            if not exists(json_path) or not exists(pred_path):
                logger.warn(
                    "Skipping because of missing files:\n\t%s" % batch_dir)
                continue
