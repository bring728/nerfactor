...
class Model(ShapeModel):
    def __init__(self, config, debug=False):
        ...
        ambi_inten = self.config.getfloat('DEFAULT', 'ambient_inten', fallback=0)
        # (1) OLAT
        ...
        self.embed_light_h = self.config.getint('DEFAULT', 'embed_light_h', fallback=32)
    ...
    def get_mlv(self, bg_ckpt, mlv_ckpt, theta_res=32, phi_res=16, r_res=128):
        model_mlv = MLV(bg_ckpt, theta_res, phi_res, r_res)
        ioutil.restore_model_v2(model_mlv, mlv_ckpt)
        model_mlv.trainable = False
        model_mlv.cube_to_mlv()
        self.model_mlv = model_mlv
        self.is_set_mlv = True

    def call(self, batch, mode='train', relight_olat=False, relight_probes=False,
             albedo_scales=None, albedo_override=None, brdf_z_override=None, with_mlv=False, env_pose=np.identity(4)):
        if mode == 'test':
            xyz_jitter_std = 0.0
        else:
        ...
        if albedo_override is not None:
            if tf.rank(albedo_override) == 1:
                albedo = tf.tile(albedo_override[None, :], (tf.shape(albedo)[0], 1))
            else:
        ...
        if self.pred_brdf:
            ...
            if self.normalize_brdf_z:
                ...
                if brdf_prop_jitter is not None:
                    brdf_prop_jitter = mathutil.safe_l2_normalize(brdf_prop_jitter, axis=1)
        else:
        ...
        # ------ Rendering equation
        if with_mlv:
            rgb_pred, rgb_olat, rgb_probes = self._render_mlv(lvis_pred, brdf, surf2l, normal_pred, env_pose)  # all Nx3
        else:
            rgb_pred, rgb_olat, rgb_probes = self._render(lvis_pred, brdf, surf2l, normal_pred, relight_olat=relight_olat,
                                                          relight_probes=relight_probes)  # all Nx3
        ...
        if rgb_olat is not None:
            rgb_olat = tf.scatter_nd(ind, rgb_olat, (n, len(self.novel_olat), 3))
        if rgb_probes is not None:
            rgb_probes = tf.scatter_nd(ind, rgb_probes, (n, len(self.novel_probes), 3))
        ...
        loss_kwargs = {'mode': mode, 'normal_jitter': normal_jitter,
    ...
    def _render_mlv(self, light_vis, brdf, l, n, env_pose):
        linear2srgb = self.config.getboolean('DEFAULT', 'linear2srgb')
        # light = self.light
        light = self.model_mlv(env_pose)[0, :, :, :3]
    ...
    def _render(self, light_vis, brdf, l, n, relight_olat=False, relight_probes=False,
        ...
        if relight_probes:
            ...
            rgb_probes = tf.debugging.check_numerics(rgb_probes, "Light Probe Renders")
    ...
    def vis_batch(self, data_dict, outdir, mode='train', dump_raw_to=None,
        ...
        def composite_on_avg_light(render, light_uint):
            ...
            bg = np.tile(avg_light[None, None, :], v_relit.shape[:2] + (1,))
        ...
        for k, v in data_dict.items():
            ...
            if k == 'pred_rgb_olat':  # HxWxLx3
                ...
                olat_n = 0
                for i, lname in enumerate(tqdm(olat_names, desc="Writing OLAT-Relit Results")):
                    ...
                    img_dict[k_relit] = xm.io.img.write_arr(img, join(outdir, k_relit + '.png'), clip=True)
            ...
            # RGB
                ...
                img_dict[k] = xm.io.img.write_arr(img, join(outdir, k + '.png'), clip=True)
            # Normals
            ...
            # Light visibility
                ...
                bg = np.ones_like(mean) if self.white_bg else np.zeros_like(mean)
                img = imgutil.alpha_blend(mean, alpha, bg)
                img_dict[k] = xm.io.img.write_arr(img, join(outdir, k + '.png'), clip=True)
                # Optionally, visualize per-light vis.
                if olat_vis:
                    for i in tqdm(range(4 if self.debug else v.shape[2] // 2),  # half
                        ...
                        img_dict[k_olat] = xm.io.img.write_arr(img, join(outdir, k_olat + '.png'), clip=True)
            # BRDF property
            elif k.endswith('brdf'):
        ...
        im1 = xm.vis.text.put_text(img_dict['gt_rgb'], "Ground Truth", **put_text_kwargs)
        im2 = xm.vis.text.put_text(img_dict['pred_rgb'], "Prediction", **put_text_kwargs)
        xm.vis.anim.make_anim((im1, im2), outpath=join(outdir, 'pred-vs-gt_rgb.apng'))
        if self.shape_mode != 'nerf':
            im1 = xm.vis.text.put_text(img_dict['gt_normal'], "NeRF", **put_text_kwargs)
            im2 = xm.vis.text.put_text(img_dict['pred_normal'], "Prediction", **put_text_kwargs)
            xm.vis.anim.make_anim((im1, im2), outpath=join(outdir, 'pred-vs-gt_normal.apng'))
            im1 = xm.vis.text.put_text(img_dict['gt_lvis'], "NeRF", **put_text_kwargs)
            im2 = xm.vis.text.put_text(img_dict['pred_lvis'], "Prediction", **put_text_kwargs)
            xm.vis.anim.make_anim((im1, im2), outpath=join(outdir, 'pred-vs-gt_lvis.apng'))
    ...
    def _compile_into_video(self, batch_dirs, out_mp4, fps=12):
        ...
        for batch_dir in tqdm(batch_dirs, desc="View Synthesis"):
            frame = visutil.make_frame(batch_dir, (('normal', 'lvis', 'nn'), ('brdf', 'albedo', 'rgb')),
        ...
        for lvis_path in tqdm(lvis_paths, desc="Final View, OLAT"):
            ...
            if self.debug and (olat_id not in self.novel_probes_uint):
                continue
            frame = visutil.make_frame(relight_view_dir, (('normal', f'lvis_olat_{olat_id}', 'nn'),
        ...
        map_i = 0
        for view_i, batch_dir in enumerate(tqdm(batch_dirs_roundtrip, desc="View Roundtrip, IBL")):
            envmap_name = envmap_names[map_i]
            frame = visutil.make_frame(batch_dir,
                                       (('normal', 'lvis', 'nn'), ('brdf', 'albedo', f'rgb_probes_{envmap_name}')),
                                       data_root=data_root, put_text_param=self.put_text_param,
