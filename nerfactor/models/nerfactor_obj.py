...
class Model(ShapeModel):
    ...
    def call(self, batch, mode='train', relight_olat=False, relight_probes=False,
        ...
        id_, hw, rayo, _, rgb, alpha, xyz_local, xyz_world, normal, lvis = batch
        # Mask out 100% background
        mask = alpha[:, 0] > 0
        rayo = tf.boolean_mask(rayo, mask)
        rgb = tf.boolean_mask(rgb, mask)
        xyz_world = tf.boolean_mask(xyz_world, mask)
        xyz_local = tf.boolean_mask(xyz_local, mask)
        normal = tf.boolean_mask(normal, mask)
        lvis = tf.boolean_mask(lvis, mask)

        # Directions
        surf2l = self._calc_ldir(xyz_world)
        surf2c = self._calc_vdir(rayo, xyz_world)
        ...
        # ------ Albedo
        albedo = self._pred_albedo_at(xyz_local)
        if xyz_noise is None:
            albedo_jitter = None
        else:
            albedo_jitter = self._pred_albedo_at(xyz_local + xyz_noise)
        ...
        # ------ BRDFs
        if self.pred_brdf:
            brdf_prop = self._pred_brdf_at(xyz_local)
            if xyz_noise is None:
                brdf_prop_jitter = None
            else:
                brdf_prop_jitter = self._pred_brdf_at(xyz_local + xyz_noise)
            ...
            brdf_prop = self._get_default_brdf_at(xyz_local)
            brdf_prop_jitter = None
    ...
    def _compile_into_video(self, batch_dirs, out_mp4, fps=12):
        ...
        for view_i, batch_dir in enumerate(tqdm(batch_dirs_roundtrip, desc="View Roundtrip, IBL")):
            ...
            frame = visutil.make_frame(batch_dir, (('normal', 'lvis', 'nn'),
