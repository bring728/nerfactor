...
class Embedder_ipe:
    def __init__(self, in_dims=3, log2_max_freq=3, n_freqs=4,
                 log_sampling=True, periodic_func=None):
        if periodic_func is None:
            periodic_func = [tf.math.sin, tf.math.cos]
        embed_func = []
        ...
        for freq in freq_bands:
            for p_f in periodic_func:
                embed_func.append(
                    lambda y, w, p_f=p_f, freq=freq: tf.math.exp(-0.5 * w * (freq ** 2)) * p_f(
                        y * freq))
                out_dims += in_dims
        self.out_dims = out_dims
        self.embed_func = embed_func

    def __call__(self, y, w):
        return tf.concat([f(y, w) for f in self.embed_func], -1)
