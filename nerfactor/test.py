...
ckpt = root_dir + 'output/train/' + scene + '/' + scene + '_nerfactor/lr5e-3/checkpoints/ckpt-15'

flags.DEFINE_string('ckpt', ckpt, "path to checkpoint (prefix only)")
flags.DEFINE_boolean('color_correct_albedo', False, "")
flags.DEFINE_integer('sv_axis_i', 0, "along which axis we do spatially-varying edits")
flags.DEFINE_float('sv_axis_min', -1.5, "axis minimum for spatially-varying edits")
flags.DEFINE_float('sv_axis_max', 1.5, "axis maximum for spatially-varying edits")
...
def main(_):
    ...
    for batch_i, batch in enumerate(tqdm(datapipe, desc="Inferring Views", total=n_views)):
        ...
        if hasattr(dataset, 'dict_mask'):
            to_vis['mask'] = dataset.dict_mask[to_vis['id'].numpy()[0].decode()]
